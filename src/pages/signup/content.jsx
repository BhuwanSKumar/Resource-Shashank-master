...
function SignForm() {
  ...
  // Regex for the institute email id (assuming it follows a pattern like "shashank.23b0131291@abes.ac.in")
  const validEmail = /^[a-zA-Z0-9._%+-]+@abes.ac.in$/;
  ...
  const signupuser = async (emailid, password, confirmpassword, username) => {
    if (!validEmail.test(emailid)) {
      ...
      setErrorr('Password and confirm password should match.');
      ...
      try {
        const userCredential = await createUserWithEmailAndPassword(emailid, password);
      ...
      <div className="bg-white rounded-lg shadow-lg w-96 border-double border-4 border-sky-500" >
        ...
        <form className="px-6 py-4">
          ...
          <div className="mb-6">
            ...
            <input type="text" className="w-full border-b-2 border-gray-300 focus:border-blue-500 outline-none dark:text-[#1A1A1C]" required onChange={(e) => setUsername(e.target.value)} />
          </div>
          <div className="mb-6">
            <label className="text-gray-500">Institute Email ID</label>
            <input type="email" className={`w-full border-b-2 border-gray-300 focus:border-blue-500 outline-none dark:text-[#1A1A1C] ${(errorcause === 'email') ? 'border-red-500' : ''}`} required onChange={(e) => setEmailid(e.target.value)} />
          </div>
          <div className="mb-6">
            ...
            <input type="password" className={`w-full border-b-2 border-gray-300 focus:border-blue-500 outline-none dark:text-[#1A1A1C] ${(errorcause === 'password') ? 'border-red-500' : ''}`} required onChange={(e) => setPassword(e.target.value)} />
          </div>
          <div className="mb-6">
            <label className="text-gray-500">Confirm Password</label>
            <input type="password" className={`w-full border-b-2 border-gray-300 focus:border-blue-500 outline-none dark:text-[#1A1A1C] ${(errorcause === 'password') ? 'border-red-500' : ''}`} required onChange={(e) => setConfirmPassword(e.target.value)} />
          </div>
          ...
          {loading ?
            ...
            <input
              ...
              onClick={(e) => {
                ...
                setErrorr(null);
                setErrorcause('');
                signupuser(emailid, password, confirmpassword, username);
              }}
            />}
